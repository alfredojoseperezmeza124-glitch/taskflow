/* ═══════════════════════════════════════════════════
   TaskFlow — modules/dashboard/dashboard.js
   Módulo Dashboard con filtros jerárquicos y métricas.
   Usa DashboardAdapter para normalizar los datos.
════════════════════════════════════════════════════ */

(function () {
  "use strict";

  const adapter = window.TF.modules.dashboardAdapter;
  const payload = window.TF.modules.dashboardPayload;
  ...
  async function cargarDashboard() {
    ...
    if (saludo) {
      ...
      const turno = hora < 12 ? "Buenos días" : hora < 18 ? "Buenas tardes" : "Buenas noches";
      const usuario = state.get("usuario") || {};
      saludo.textContent = `${turno}, ${(usuario.nombre || "").split(" ")[0]}`;
    }

    const acciones = document.getElementById("dashAcciones");
    const rol = state.get("usuario")?.rol;
    if (acciones && (rol === "PROJECT_MANAGER" || rol === "ADMIN")) {
      acciones.innerHTML =
    ...
    _dashTimer = setInterval(() => {
      if (document.getElementById("pantalla-dashboard")?.classList.contains("activa")) {
  ...
  async function refrescarDashboard() {
    ...
    if (window.toast) window.toast("Dashboard actualizado");
  }
  ...
  async function _poblarSelectorDash() {
    ...
    try {
      ...
      _dashProyectos = (res || []).map(p => adapter.adaptProyecto(p));
      sel.innerHTML =
      ...
      const seleccionValida = !_dashProyId || _dashProyectos.some((p) => p.id === _dashProyId);
  ...
  async function actualizarDashboardProy(proyId) {
    ...
    if (_dashProyId) state.set("proyectoActualId", _dashProyId);
    await _cargarMetricasDash(_dashProyId, false);
  }
  ...
  async function _dashObtenerMapaUsuarios() {
    if (typeof window._obtenerMapaUsuarios === "function") {
      try {
        return await window._obtenerMapaUsuarios();
      } catch (_) {}
    }
    try {
      const lista = await window.api("GET", "/usuarios/activos");
      const mapa = {};
      (lista || []).forEach((u) => {
        mapa[u.id] = u;
      });
  ...
  async function _dashListarTodasTareasProyecto(proyId) {
    ...
    return tareas.map(t => adapter.adaptTarea(t));
  }
  ...
  async function _dashConstruirDataProyecto(proyecto) {
    ...
    if (typeof proyecto === "object" && proyecto?.id) {
      base.proyectosPorId[proyecto.id] = adapter.adaptProyecto(proyecto);
    }

    const [tableros, estructura, subtareasEtapa, tareas] = await Promise.all([
      window.api("GET", `/proyectos/${proyId}/tableros`).catch(() => []),
    ...
    base.subtareasEtapa = (Array.isArray(subtareasEtapa) ? subtareasEtapa : []).map(s => adapter.adaptSubtareaEtapa(s));
  ...
  async function _dashCargarBase(proyId, forzar = false) {
    ...
    const data = proyId
      ? await _dashConstruirDataProyecto(_dashProyectos.find((p) => p.id === proyId) || { id: proyId })
  ...
  function _dashPoblarFasesEtapas(base) {
    ...
    selFase.innerHTML =
      ...
      fases
        .map((fase) => {
          ...
          const sufijo = base?.esGlobal && nombreProyecto ? ` · ${nombreProyecto}` : "";
          return `<option value="${fase.id}">${_dashEsc((fase.nombre || "Fase") + sufijo)}</option>`;
        })
        .join("");
    selFase.value = fases.some((fase) => fase.id === faseActual) ? faseActual : "";
    ...
    selEtapa.innerHTML =
      ...
      etapas
        ...
        .join("");
    selEtapa.value = etapas.some((etapa) => etapa.id === etapaActual) ? etapaActual : "";
  ...
  function _dashPoblarResponsables(base) {
    ...
    const ids = new Set();
    (base?.tareas || []).forEach((t) => (t.responsables || []).forEach((id) => ids.add(id)));
    (base?.subtareasEtapa || []).forEach((s) => (s.responsables || []).forEach((id) => ids.add(id)));
  ...
  function _leerFiltrosDash() {
    return payload.filtros({
      ...
      incluyeSubEtapa: document.getElementById("chkDashIncluirSubEtapa")?.checked !== false,
    });
  }
  ...
  function _dashFiltrarData(base, filtros) {
    const tareas = (base?.tareas || []).filter((t) => {
      ...
      if (filtros.responsableId && !(t.responsables || []).includes(filtros.responsableId)) {
        return false;
      }
    ...
    const subtareasEtapa = (base?.subtareasEtapa || []).filter((s) => {
      ...
      if (filtros.faseId && faseId !== filtros.faseId) return false;
      if (filtros.etapaId && etapaId !== filtros.etapaId) return false;
      if (filtros.responsableId && !(s.responsables || []).includes(filtros.responsableId)) {
        return false;
      }
      return true;
    });
  ...
  function _dashConstruirResumen(base, filtrado, filtros) {
    ...
    const subtareasEtapa = filtros.incluyeSubEtapa ? filtrado.subtareasEtapa || [] : [];
    ...
    const estructuradas = tareas.filter((t) => !!(_dashFaseDesdeTarea(base, t) || t.etapaId)).length;
    const legadas = totalTareas - estructuradas;
    const coberturaEstructura = totalTareas ? Math.round((estructuradas / totalTareas) * 100) : 0;
    const porcentajeCompletado = totalTrabajo ? Math.round((completadasTrabajo / totalTrabajo) * 100) : 0;
  ...
  function _dashPintarKpis(resumen, filtrado, filtros) {
    _setKpi("stDashTrabajo", resumen.totalTrabajo, `${resumen.porcentajeCompletado}% completado`);
    _setKpi("stDashTareas", resumen.totalTareas, `${resumen.enProgresoTareas} en progreso`);
    _setKpi(
      "stDashSubEtapas",
      resumen.totalSubEtapa,
      filtros.incluyeSubEtapa ? `${resumen.completadasSubEtapa} completadas` : "excluidas del consolidado",
    );
    _setKpi("stDashVencidas", resumen.vencidas, resumen.vencidas > 0 ? "requieren atención" : "al día ✓");
    _setKpi(
      "stDashCobertura",
  ...
  function _dashRenderInsights(base, filtrado, resumen, filtros) {
    ...
    if (filtros.incluyeSubEtapa) {
      ...
      const [etapaTop, pendientes] = _dashTop(Object.entries(pendientesPorEtapa), 1)[0] || ["", 0];
      if (pendientes > 0) {
        alertas.push(
    ...
    const [responsableTop, cargaTop] = _dashTop(Object.entries(carga), 1)[0] || ["", 0];
    if (cargaTop > 0) {
      const nombreResp =
        responsableTop === "__sin_responsable__"
          ? "Sin responsable"
          : base?.usuariosPorId?.[responsableTop]?.nombre || `Usuario ${String(responsableTop).slice(-6)}`;
      alertas.push(`👥 Mayor carga actual: <strong>${_dashEsc(nombreResp)}</strong> con <strong>${cargaTop}</strong> asignaciones.`);
    }

    if (!alertas.length) {
      cont.innerHTML = '<div class="vacío">Sin datos suficientes para generar insights.</div>';
      return;
    }
  ...
  function _renderizarGraficos(base, filtrado, resumen, filtros) {
    ...
    if (filtros.incluyeSubEtapa) {
      const pendientes = (filtrado.subtareasEtapa || []).filter((s) => !s.completada).length;
      const completadas = (filtrado.subtareasEtapa || []).filter((s) => s.completada).length;
      if (pendientes > 0) estadoMap["Subtareas etapa · Pendientes"] = pendientes;
      if (completadas > 0) estadoMap["Subtareas etapa · Completadas"] = completadas;
    }
    const estadoEntradas = _dashTop(Object.entries(estadoMap), 12);
    const estadoCategorias = estadoEntradas.length ? estadoEntradas.map(([k]) => k) : ["Sin datos"];
    const estadoValores = estadoEntradas.length ? estadoEntradas.map(([, v]) => v) : [0];
    ...
    const serieTareas = _dashContarPorVentana(filtrado.tareas || [], semanas, (t) => t.creadoEn);
    const serieSubtareas = _dashContarPorVentana(filtrado.subtareasEtapa || [], semanas, (s) => s.creadoEn);
    const seriesVelocidad = [{ name: "Tareas", data: serieTareas, color: "#6366f1" }];
    if (filtros.incluyeSubEtapa) {
    ...
    const faseCategorias = faseEntradas.length ? faseEntradas.map(([k]) => k) : ["Sin datos"];
    const faseValores = faseEntradas.length ? faseEntradas.map(([, v]) => v) : [0];
    ...
    const etapaCategorias = etapaEntradas.length ? etapaEntradas.map(([k]) => k) : ["Sin datos"];
    const etapaTareas = etapaCategorias.map((nombre) => etapaMap[nombre]?.tareas || 0);
    const etapaSubtareas = etapaCategorias.map((nombre) => etapaMap[nombre]?.subtareas || 0);
    ...
    const cargaCategorias = cargaEntradas.length
      ? cargaEntradas.map(([id]) =>
          id === "__sin_responsable__"
            ? "Sin responsable"
            : base?.usuariosPorId?.[id]?.nombre || `Usuario ${String(id).slice(-6)}`,
        )
      : ["Sin datos"];
    const cargaValores = cargaEntradas.length ? cargaEntradas.map(([, v]) => v) : [0];
    ...
    const prioEntradas = ordenPrio.map((k) => [k, prioMap[k] || 0]).filter(([, v]) => v > 0);
    const prioColores = {
      BAJA: "#22c55e",
      MEDIA: "#f59e0b",
      ALTA: "#ef4444",
    ...
    const tipoEntradas = ordenTipo.map((k) => [k, tipoMap[k] || 0]).filter(([, v]) => v > 0);
    const tipoCategorias = tipoEntradas.length ? tipoEntradas.map(([k]) => k) : ["Sin datos"];
    const tipoValores = tipoEntradas.length ? tipoEntradas.map(([, v]) => v) : [0];
    const tipoColores = {
      TASK: "#a1a1aa",
      BUG: "#ef4444",
      FEATURE: "#06b6d4",
  ...
  async function _cargarProyectosRecientes() {
    ...
    try {
      const res = await window.api("GET", "/proyectos/");
      const ps = (res || []).map(p => adapter.adaptProyecto(p));
      const accionesProy = document.getElementById("dashAccionesProy");
      const usuario = state.get("usuario") || {};
      if (accionesProy && (usuario.rol === "PROJECT_MANAGER" || usuario.rol === "ADMIN")) {
        ...
        ? ps
            ...
            <div class="proy-row">
              ...
              <div class="proy-info">
                ...
                <div class="proy-meta">${_dashEsc(p.descripcion || "Sin descripción")} · fin ${window.fFecha ? window.fFecha(p.fechaFinEstimada) : p.fechaFinEstimada}</div>
  ...
  function iniciarInputMenciones(inputId, sugerenciasId) {
    ...
    input.addEventListener("input", async () => {
      ...
      const filtrados = _usuariosMencion
        .filter((u) => u.token.includes(query) || u.email.toLowerCase().includes(query))
        .slice(0, 6);
      ...
      sug.innerHTML = filtrados
        .map(
          (u, i) => `
        <div class="sug-item" data-i="${i}" data-nombre="${u.nombre}" onclick="window._insertarMencion('${inputId}','${sugerenciasId}','${u.nombre}')">
          <div class="avatar avatar-sm">${window.inic ? window.inic(u.nombre) : ""}</div>
          <div>
        ...
        <div class="modal" style="width:580px;max-height:80vh;display:flex;flex-direction:column">
          ...
          <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--borde)">
            ...
            <div class="flex-between" style="margin-top:8px">
              ...
              <button class="btn btn-primary btn-sm" onclick="window.enviarComentario()">Enviar</button>
            </div>
          </div>
        </div>`;
      document.body.appendChild(modal);
      modal.addEventListener("click", (e) => {
        if (e.target === modal) window.cerrarModal("mComentarios");
      });
    }

    document.getElementById("mComTitulo").textContent = `Comentarios · ${tituloTarea}`;
    window.abrirModal("mComentarios");
    await _cargarUsuariosMencion();
  ...
  async function _cargarComentariosPaginados(tareaId, pagina) {
    ...
    try {
      ...
      const comentarios = res.datos || res;

      if (!comentarios.length) {
        lista.innerHTML = '<div class="vacío">Sin comentarios aún. ¡Sé el primero!</div>';
      } else {
        const mapaU = await _dashObtenerMapaUsuarios();
        lista.innerHTML = comentarios
          .map((c) => {
            const u = mapaU[c.autorId];
            const nombre = u ? u.nombre : `ID:${c.autorId.slice(-6)}`;
            const av = `<div class="avatar avatar-sm">${window.inic ? window.inic(nombre) : ""}</div>`;
            const html = c.contenidoHtml || c.contenido;
            return `<div class="comentario-item">
            <div class="comentario-header">
              ${av}
              <span class="comentario-autor">${nombre}</span>
              <span class="comentario-fecha">${window.fFecha ? window.fFecha(c.creadoEn) : c.creadoEn}</span>
            </div>
  ...
  async function enviarComentario() {
    ...
    try {
      await window.api("POST", `/tareas/${_tareaComentarioId}/comentarios`, {
        contenido: texto,
      });
      input.value = "";
      await _cargarComentariosPaginados(_tareaComentarioId, _paginaComentarios);
      if (window.toast) window.toast("Comentario enviado");
    } catch (e) {
  ...
  function _renderizarPaginacion(paginaActual, totalPaginas, onCambio) {
    ...
    if (inicio > 1) {
      btns.push(`<button class="pag-btn" onclick="(${onCambio.toString()})(1)">1</button>`);
    }
    ...
    if (fin < totalPaginas) {
      btns.push(`<button class="pag-btn" onclick="(${onCambio.toString()})(${totalPaginas})">${totalPaginas}</button>`);
    }
  ...
  // Exponer a global para compatibilidad con las vistas existentes
  window.cargarDashboard = cargarDashboard;
  window.refrescarDashboard = refrescarDashboard;
  window.actualizarDashboardProy = actualizarDashboardProy;
  window.onCambioFaseDashboard = onCambioFaseDashboard;
  window.onCambioFiltroDashboard = onCambioFiltroDashboard;
  window.limpiarFiltrosDashboard = limpiarFiltrosDashboard;
  window._insertarMencion = _insertarMencion;
  window.abrirPanelComentarios = abrirPanelComentarios;
  window.enviarComentario = enviarComentario;

  /* ── Registrar módulo ── */
  window.TF.moduleLoader.registrar("dashboard", {
    name: "dashboard",
    htmlPath: "js/modules/dashboard/dashboard.html",
    cssPath: null,
    adapter: adapter,
    payload: payload,
