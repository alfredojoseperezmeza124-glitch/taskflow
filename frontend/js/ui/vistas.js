...
function abrirAsistenteProyecto(proyectoId, nombreProyecto) {
  // Sin proyecto → mostrar selector en lugar de error
  const idFinal = proyectoId || proyActualId;
  if (!idFinal) {
  ...
  if (!token) { toast("Sesión no válida", "err"); return; }

  const tituloEl = document.getElementById("mAsistenteTitulo");
  if (tituloEl) tituloEl.textContent = nombreProyecto || "Proyecto";

  const contenedor = document.getElementById("mAsistenteContenedor");
  if (contenedor) {
    contenedor.innerHTML = "";
    window.renderAsistenteProyecto(contenedor, idFinal, token);
  }

  abrirModal("mAsistenteProyecto");
}

/* ── Selector de proyecto para el asistente IA ── */
async function mostrarSelectorProyecto() {
  const lista = document.getElementById("selectorProyectoLista");
  if (!lista) return;

  lista.innerHTML = '<div style="text-align:center;color:var(--t3);padding:24px"><span class="spinner"></span> Cargando…</div>';
  abrirModal("mSelectorProyecto");

  try {
    const resp = await api("GET", "/proyectos/");
    const items = Array.isArray(resp) ? resp : (resp?.proyectos || resp?.items || []);

    if (!items.length) {
      lista.innerHTML = '<div style="text-align:center;color:var(--t3);padding:24px">No tienes proyectos activos</div>';
      return;
    }

    lista.innerHTML = items.map((p) => {
      const progreso = Math.round(p.progreso ?? 0);
      const nombre = p.nombre || "Sin nombre";
      const desc = p.descripcion || "";
      return `
        <button class="sp-proyecto-item" onclick="seleccionarProyectoAsistente('${p.id}','${nombre.replace(/'/g,"\\'")}')">
          <div class="sp-proy-icon"><i class="ph ph-kanban"></i></div>
          <div class="sp-proy-info">
            <div class="sp-proy-nombre">${_escTxt(nombre)}</div>
            ${desc ? `<div class="sp-proy-desc">${_escTxt(desc.slice(0,80))}${desc.length>80?"…":""}</div>` : ""}
            <div class="sp-proy-footer">
              <div class="sp-proy-bar" title="${progreso}% completado">
                <div class="sp-proy-bar-fill" style="width:${progreso}%"></div>
              </div>
              <span style="font-size:10px;color:var(--t3);min-width:28px">${progreso}%</span>
            </div>
          </div>
          <i class="ph ph-caret-right sp-proy-arrow"></i>
        </button>`;
    }).join("");
  } catch (e) {
    lista.innerHTML = `<div style="text-align:center;color:var(--red);padding:24px">Error: ${e.message}</div>`;
  }
}

function seleccionarProyectoAsistente(proyectoId, nombreProyecto) {
  cerrarModal("mSelectorProyecto");
  if (window.AsistenteFlotante) {
    window.AsistenteFlotante.actualizarProyecto(proyectoId, nombreProyecto);
  }
  abrirAsistenteProyecto(proyectoId, nombreProyecto);
}
  ...
  try {
    ...
    document.getElementById("listaProyectos").innerHTML = ps
      .map((p) => {
        ...
        const progresoClass = progreso >= 80 ? "bg-ok" : progreso >= 40 ? "bg-brand" : "bg-warn";
        return `
      <div class="prow group relative rounded-2xl border border-white/[0.07] bg-gradient-to-br from-[var(--s2)] to-[var(--s2)] hover:border-brand/25 hover:shadow-[0_4px_24px_rgba(108,99,255,0.1)] transition-all duration-200 overflow-hidden">
        <div class="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-brand/70 via-brand-light/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
        <div class="prow-main p-4">
          <div class="prow-top flex items-start justify-between gap-3 mb-2">
            <div class="flex items-center gap-2.5 min-w-0">
              <div class="w-9 h-9 rounded-xl bg-brand/12 border border-brand/20 flex items-center justify-center flex-shrink-0 group-hover:bg-brand/20 transition-colors">
                <i class="ph ph-kanban text-brand-light"></i>
              </div>
              <div class="prow-name font-semibold text-[var(--t1)] text-sm truncate">${p.nombre}</div>
            </div>
            <div class="flex items-center gap-1.5 flex-shrink-0">
              ${badgeEstado(p.estado)}
              ${activos.length ? `<span class="badge bb text-[9px]" title="${activos.join(", ")}"><i class="ph ph-stack" style="font-size:9px"></i> ${activos.length}/3</span>` : ""}
            </div>
          </div>
          <div class="prow-desc text-xs text-[var(--t3)] leading-relaxed mb-3 line-clamp-2">${p.descripcion || "Sin descripción"}</div>
          <div class="flex items-center gap-3 mb-3 text-[10px] text-[var(--t3)]">
            <span class="flex items-center gap-1"><i class="ph ph-calendar-blank"></i> ${fFecha(p.fechaFinEstimada)}</span>
            <span class="flex items-center gap-1 font-semibold ${progreso >= 80 ? "text-ok" : progreso >= 40 ? "text-brand-light" : "text-warn"}">
              <i class="ph ph-gauge"></i> ${progreso.toFixed(1)}%
            </span>
          </div>
          <div class="space-y-1">
            <div class="flex justify-between text-[9px] text-[var(--t3)]">
              <span>Progreso del proyecto</span><span class="font-semibold">${progreso.toFixed(0)}%</span>
            </div>
            <div class="h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
              <div class="h-full rounded-full ${progresoClass} transition-all duration-700" style="width:${progreso}%"></div>
            </div>
          </div>
        </div>
        <div class="prow-actions flex flex-wrap gap-1.5 px-4 pb-4">
          <button class="btn btn-outline btn-xs rounded-lg hover:bg-brand/10 hover:border-brand/40 hover:text-brand-light transition-all" onclick="irTablero('${p.id}','${p.nombre}')"><i class='ph ph-kanban'></i> Tablero</button>
          <button class="btn btn-outline btn-xs rounded-lg hover:bg-white/[0.06] transition-all" onclick="abrirJerarquiaProyecto('${p.id}')"><i class='ph ph-tree-structure'></i> Estructura</button>
          ${
            S.usuario.rol !== "DEVELOPER"
              ? `
            <button class="btn btn-outline btn-xs rounded-lg hover:bg-white/[0.06] transition-all" onclick="abrirInvitar('${p.id}')"><i class='ph ph-user-plus'></i> Invitar</button>
            <button class="btn btn-outline btn-xs rounded-lg hover:bg-white/[0.06] transition-all" onclick="abrirReglasDecorador('${p.id}')"><i class='ph ph-sliders-horizontal'></i> Reglas</button>
            <button class="btn btn-outline btn-xs rounded-lg hover:bg-white/[0.06] transition-all" onclick="clonarProyecto('${p.id}')"><i class='ph ph-copy'></i> Clonar</button>
            <button class="btn btn-primary btn-xs rounded-lg shadow-brand/20 shadow-sm hover:shadow-brand/40 transition-all" onclick="abrirAsistenteProyecto('${p.id}','${p.nombre.replace(/'/g, "\\'")}')"><i class='ph ph-sparkle'></i> Asistente IA</button>
            ${!p.estaArchivado ? `<button class="btn btn-ghost btn-xs rounded-lg hover:text-warn hover:bg-warn/10 transition-all" onclick="archivarProyecto('${p.id}')">Archivar</button>` : ""}
          `
              : ""
          }
        </div>
