...
${contexto}`;
      ...
      <div class="ap-chat">
        <div class="ap-chat-msgs" id="ap-msgs">
          <div class="ap-bienvenida" id="ap-bienvenida">
            ...
            <div class="ap-robot" aria-hidden="true">
              <div class="ap-robot-antenna"><div class="ap-robot-antenna-ball"></div></div>
              <div class="ap-robot-head">
                <div class="ap-robot-visor">
                  <div class="ap-robot-eyes">
                    <div class="ap-robot-eye"><div class="ap-robot-pupil"></div></div>
                    <div class="ap-robot-eye"><div class="ap-robot-pupil"></div></div>
                  </div>
                </div>
                <div class="ap-robot-mouth">
                  <span></span><span></span><span></span>
                </div>
              </div>
              <div class="ap-robot-neck"></div>
              <div class="ap-robot-body">
                <div class="ap-robot-chest">
                  <div class="ap-robot-led ap-led-r"></div>
                  <div class="ap-robot-led ap-led-g"></div>
                  <div class="ap-robot-led ap-led-b"></div>
                </div>
                <div class="ap-robot-panel"></div>
              </div>
              <div class="ap-robot-arms">
                <div class="ap-robot-arm ap-arm-l"></div>
                <div class="ap-robot-arm ap-arm-r"></div>
              </div>
            </div>
            <div class="ap-bienvenida-txt">
              <strong style="color:var(--t1,#e4e6f0);font-size:14px">¡Hola! Soy tu asistente IA</strong><br>
              Puedo revisar el estado del proyecto,<br>
              crear tareas, subtareas y asignar responsables<br>
              usando lenguaje natural. ¡Pregúntame algo!
            </div>
            <div class="ap-bienvenida-caps">
              <span><i class="ph ph-plus-circle"></i> Crear tareas</span>
              <span><i class="ph ph-users"></i> Asignar responsables</span>
              <span><i class="ph ph-tree-structure"></i> Crear subtareas</span>
            </div>
          </div>
        </div>
        ...
        <div class="ap-input-wrap">
          <textarea
            class="ap-input"
            id="ap-input"
            placeholder="Pregunta algo o usa el micrófono…"
            rows="1"
          ></textarea>
          <button class="ap-btn-voz" id="ap-btn-voz" title="Dictado por voz">
            <i class="ph ph-microphone"></i>
          </button>
          <button class="ap-btn-send" id="ap-btn-send" title="Enviar (Enter)">
  ...
  if (!document.getElementById("ap-styles")) {
    ...
    s.textContent = `
      ...
      .ap-bienvenida-txt { font-size: 13px; line-height: 1.8; }
      .ap-bienvenida-caps {
        display: flex; gap: 8px; flex-wrap: wrap; justify-content: center;
      }
      .ap-bienvenida-caps span {
        display: flex; align-items: center; gap: 4px;
        background: rgba(91,82,232,.1); border: 1px solid rgba(91,82,232,.2);
      ...
      .ap-robot {
        ...
        animation: ap-robot-float 3s ease-in-out infinite;
        filter: drop-shadow(0 8px 24px rgba(91,82,232,.35));
      }
      @keyframes ap-robot-float {
        0%,100% { transform: translateY(0); }
        50%      { transform: translateY(-8px); }
      }
      .ap-robot-antenna {
        width: 3px; height: 14px; background: #6c63ff;
        margin: 0 auto; border-radius: 2px;
      }
      .ap-robot-antenna-ball {
        width: 8px; height: 8px; border-radius: 50%;
        background: #ff6b9d; margin: -4px auto 0;
        box-shadow: 0 0 8px rgba(255,107,157,.8);
        animation: ap-led-blink 1.2s ease-in-out infinite;
      }
      .ap-robot-head {
        width: 72px; height: 52px; background: linear-gradient(145deg, #2a2560, #1a1a3e);
      ...
      .ap-robot-head::before {
        content: ''; position: absolute; inset: 0;
        background: linear-gradient(180deg, rgba(255,255,255,.05) 0%, transparent 60%);
      }
      .ap-robot-visor {
        margin: 10px 8px 0; padding: 6px 8px;
        background: rgba(0,0,0,.3); border-radius: 8px;
        border: 1px solid rgba(108,99,255,.3);
      }
      .ap-robot-eyes { display: flex; gap: 10px; justify-content: center; }
      .ap-robot-eye {
        width: 14px; height: 14px; border-radius: 50%;
        background: #111; border: 2px solid #6c63ff;
        display: flex; align-items: center; justify-content: center;
        position: relative;
        animation: ap-eye-blink 4s ease-in-out infinite;
      }
      .ap-robot-eye:nth-child(2) { animation-delay: .15s; }
      .ap-robot-pupil {
        width: 6px; height: 6px; border-radius: 50%;
        background: radial-gradient(circle at 30% 30%, #8b83ff, #5b52e8);
        box-shadow: 0 0 4px rgba(108,99,255,.9);
      }
      @keyframes ap-eye-blink {
      ...
      .ap-robot-mouth span {
        ...
        animation: ap-mouth-wave 1.6s ease-in-out infinite;
      }
      .ap-robot-mouth span:nth-child(2) { animation-delay: .2s; }
      .ap-robot-mouth span:nth-child(3) { animation-delay: .4s; }
      @keyframes ap-mouth-wave {
        0%,100% { transform: translateY(0); opacity: .6; }
        50%      { transform: translateY(-2px); opacity: 1; }
      }
      .ap-robot-neck {
        width: 16px; height: 6px; background: rgba(108,99,255,.3);
        margin: 0 auto; border-radius: 0 0 4px 4px;
      }
      .ap-robot-body {
        width: 64px; height: 44px; margin: 0 auto;
        background: linear-gradient(145deg, #1e1b4b, #16163a);
        border-radius: 10px; border: 2px solid rgba(108,99,255,.4);
        padding: 8px; box-shadow: inset 0 1px 0 rgba(255,255,255,.06);
        position: relative;
      }
      .ap-robot-chest { display: flex; gap: 5px; justify-content: center; margin-bottom: 5px; }
      .ap-robot-led {
        width: 7px; height: 7px; border-radius: 50%;
        animation: ap-led-blink 1.4s ease-in-out infinite;
      }
      .ap-led-r { background: #f87171; box-shadow: 0 0 5px rgba(248,113,113,.8); }
      .ap-led-g { background: #4ade80; box-shadow: 0 0 5px rgba(74,222,128,.8); animation-delay: .5s; }
      .ap-led-b { background: #60a5fa; box-shadow: 0 0 5px rgba(96,165,250,.8); animation-delay: 1s; }
      @keyframes ap-led-blink {
        0%,100% { opacity: 1; } 50% { opacity: .3; }
      }
      .ap-robot-panel {
        height: 12px; background: rgba(0,0,0,.3);
        border-radius: 4px; border: 1px solid rgba(108,99,255,.2);
      }
      .ap-robot-arms { display: flex; justify-content: space-between; margin-top: -44px; padding: 0 -4px; }
      .ap-robot-arm {
        width: 10px; height: 34px; background: linear-gradient(180deg,rgba(108,99,255,.4),rgba(108,99,255,.2));
        border-radius: 5px; border: 1px solid rgba(108,99,255,.3);
      }
      .ap-arm-l { transform: rotate(12deg); margin-left: -8px; }
      .ap-arm-r { transform: rotate(-12deg); margin-right: -8px; }

      /* ── Botón de voz ── */
      .ap-btn-voz {
        width: 38px; height: 38px; border-radius: 10px; border: 1px solid var(--b2, rgba(255,255,255,.12)); flex-shrink: 0;
        background: var(--s2, rgba(255,255,255,.06)); color: var(--t2, #adb3cc); cursor: pointer;
        display: flex; align-items: center; justify-content: center;
        transition: all .15s; font-size: 16px;
      }
      .ap-btn-voz:hover { background: rgba(91,82,232,.15); color: var(--a, #8b83ff); border-color: rgba(91,82,232,.3); }
      .ap-btn-voz.grabando {
        background: rgba(248,113,113,.15); color: #f87171;
        border-color: rgba(248,113,113,.5);
        animation: ap-voz-pulse 1s ease-in-out infinite;
      }
      @keyframes ap-voz-pulse {
  ...
  /* ── Voz: Web Speech API ─────────────────────────────────── */
  (function iniciarVoz() {
    const vozBtn = document.getElementById("ap-btn-voz");
    if (!vozBtn) return;
    ...
    if (!SpeechRecognition) {
      vozBtn.title = "Voz no disponible en este navegador";
      vozBtn.style.opacity = ".35";
      vozBtn.style.cursor = "not-allowed";
      return;
    }
    ...
    recog.onstart = () => {
      ...
      vozBtn.classList.add("grabando");
      vozBtn.innerHTML = '<i class="ph ph-stop-circle"></i>';
      vozBtn.title = "Detener dictado";
    ...
    recog.onresult = (e) => {
      ...
      inputEl.value = (textoBase + " " + final + interino).trim();
      inputEl.style.height = "auto";
    ...
    recog.onend = () => {
      grabando = false;
      vozBtn.classList.remove("grabando");
      vozBtn.innerHTML = '<i class="ph ph-microphone"></i>';
      vozBtn.title = "Dictado por voz";
      inputEl.placeholder = "Pregunta algo o usa el micrófono…";
      inputEl.focus();
    };

    recog.onerror = (e) => {
      if (e.error !== "aborted") {
        inputEl.placeholder = "Error de micrófono: " + e.error;
        setTimeout(() => {
          inputEl.placeholder = "Pregunta algo o usa el micrófono…";
        }, 2500);
      }
