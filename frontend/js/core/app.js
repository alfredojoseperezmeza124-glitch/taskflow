...
/* ── Caché de peticiones GET (Patrón Cache-Aside) ── */
const _apiCache = new Map();
const _API_CACHE_TTL = 30_000; // 30 s
const _pendingRequests = new Map(); // deduplicación

function _cacheKey(ruta) { return ruta; }

function _cacheGet(key) {
  const entry = _apiCache.get(key);
  if (!entry) return null;
  if (Date.now() - entry.ts > _API_CACHE_TTL) { _apiCache.delete(key); return null; }
  return entry.data;
}
...
function invalidarCache(patronRuta) {
  for (const k of _apiCache.keys()) {
    if (k.includes(patronRuta)) _apiCache.delete(k);
  }
}

/* ── HTTP (con caché para GET + deduplicación) ── */
async function api(met, ruta, body = null, token = true) {
  const h = { "Content-Type": "application/json" };
  if (token && S?.token_acceso) h["Authorization"] = `Bearer ${S.token_acceso}`;

  // Caché solo para GET sin body
  if (met === "GET" && !body) {
    const key = _cacheKey(ruta);
    const cached = _cacheGet(key);
    if (cached) return cached;

    // Deduplicar: si ya hay una petición en vuelo para esta ruta, esperar la misma
    if (_pendingRequests.has(key)) return _pendingRequests.get(key);

    const promise = fetch(`${API}${ruta}`, { method: "GET", headers: h })
      .then(async (r) => {
      ...
      .catch((err) => { _pendingRequests.delete(key); throw err; });

    _pendingRequests.set(key, promise);
    return promise;
  }

  // POST / PUT / DELETE: invalidar caché del recurso afectado
  if (met !== "GET") {
    const recurso = ruta.split("/")[1]; // ej: "tareas", "proyectos"
    if (recurso) invalidarCache("/" + recurso);
  }
...
async function iniciarSesion() {
  ...
  try {
    ...
    // ── Notificar componentes observadores (FAB, etc.) ──
    window.dispatchEvent(new CustomEvent("taskflow:sesion-iniciada", { detail: { usuario: S.usuario } }));
...
function cerrarSesion() {
  ...
  // Notificar componentes observadores (FAB, etc.)
  window.dispatchEvent(new CustomEvent("taskflow:sesion-cerrada"));
  ...
  document.addEventListener("taskflow:ready", () => {
    ...
    if (saved) {
      try {
        ...
        // Notificar componentes observadores (FAB, etc.) que hay sesión activa
        window.dispatchEvent(
          new CustomEvent("taskflow:sesion-iniciada", { detail: { usuario: S.usuario } })
        );
